            ...
            for line in lines:
                ...
                for urls in all_urls:
                    ...
                    if name == movie_name and year == year_0:
                        time.sleep(1)
                        count = 1
                        Count = Count + get_company(urls, movie_name, id,year_0)
                        time.sleep(1)
                        break
                    else:
                        pass
                if count == 0:
                    with open('./movie_failed_name.txt', "a+")as w:
                        w.writelines(year_0 + '' + movie_name + '' + id )
                        w.write('\n')
        with open("./count_file.txt","a+") as w:
             w.writelines(year_0 + ' ' + str(Count) + '\n')
...
def get_company(urls, movie_name,id,year_0):
    ...
    try:
        ...
        with open('./movie_lack_' + year_0 + '.txt',"a+")as w:
    ...
    res = json.dumps(res)
    s = json.loads(res)
    flag = mongoDB_insert(id, movie_name, s,p_company,l_company,year_0)
    return flag
...
def mongoDB_insert(id,movie_name,s,p_company,l_company,year_0):
    ...
    if l_company == 1 and p_company == 1:
        try:
            table.insert({'_id': id, 'year': year_0,
            ...
            with open('./double_id.txt','a+') as w:
                w.writelines(year_0 + movie_name + ' ' + id + '\n')
            return 0
